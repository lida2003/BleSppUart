...
#include "ble_spp_uart.h"

#define GATTS_TABLE_TAG  "BleSppUart"

#define SPP_PROFILE_NUM             1
#define SPP_PROFILE_APP_IDX         0
#define ESP_SPP_APP_ID              0x56
#define SAMPLE_DEVICE_NAME          GATTS_TABLE_TAG    //The Device Name Characteristics in GAP
...
static const uint8_t spp_adv_data[23] = {
    ...
    0x0F,0x09, 'B', 'L', 'E', '_', 'S', 'P', 'P', '_', 'U', 'A', 'R','T', ' ', ' '
};
        ...
        if (xQueueReceive(spp_uart_queue, (void * )&event, (TickType_t)portMAX_DELAY)) {
            ...
            case UART_DATA:
                if ((event.size)&&(is_connected)) {
					...
					ESP_LOGI(GATTS_TABLE_TAG, "Debug read %d Bytes\n", event.size);
					ESP_LOG_BUFFER_HEX(GATTS_TABLE_TAG, temp, event.size);
        ...
        if (xQueueReceive(ttl_uart_queue, (void * )&event, (TickType_t)portMAX_DELAY)) {
            ...
            case UART_DATA:
                if ((event.size)&&(is_connected)) {
					...
					ESP_LOGI(GATTS_TABLE_TAG, "ttl read %d Bytes\n", event.size);
					ESP_LOG_BUFFER_HEX(GATTS_TABLE_TAG, temp, event.size);
                    ...
                    if(event.size <= (spp_mtu_size - 3)){
                        ...
                        while(current_num <= total_num){
                            if(current_num < total_num){
								...
								ESP_LOGI(GATTS_TABLE_TAG, "U1 Pack #%d size %d\n", current_num, (spp_mtu_size-3));
							ESP_LOG_BUFFER_HEX(GATTS_TABLE_TAG, ntf_value_p, (spp_mtu_size-3));
                            }else if(current_num == total_num){
                                ntf_value_p[0] = '#';
                                ntf_value_p[1] = '#';
                                ntf_value_p[2] = total_num;
								...
								ESP_LOGI(GATTS_TABLE_TAG, "U1 Pack #%d size %d\n", current_num, (event.size - (current_num - 1)*(spp_mtu_size - 7) + 4));
							ESP_LOG_BUFFER_HEX(GATTS_TABLE_TAG, ntf_value_p, (event.size - (current_num - 1)*(spp_mtu_size - 7) + 4));
                            }
...
void ttl_uart_init(void)
{
    ...
    uart_driver_install(TTL_UART_NUM, TTL_UART_BUF_SIZE * 2, 8192, 10,&ttl_uart_queue,0);
    //Set UART parameters
...
void spp_uart_init(void)
{
    ...
    xTaskCreate(debug_uart_task, "debug_uart_task", 2048, (void*)UART_NUM_0, 8, NULL);
}
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    ESP_LOGI(GATTS_TABLE_TAG, "BLE read %d Bytes\n", p_data->write.len);
					ESP_LOG_BUFFER_HEX(GATTS_TABLE_TAG, p_data->write.value, p_data->write.len);
                    uart_write_bytes(TTL_UART_NUM, (char *)(p_data->write.value), p_data->write.len);
                }else{
                    //TODO:
                }
